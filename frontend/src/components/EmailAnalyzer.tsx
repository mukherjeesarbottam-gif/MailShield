...
interface EmailAnalyzerProps {
  onStatusChange?: (status: 'IDLE' | 'ANALYZING' | 'RESULT' | 'ERROR') => void;
}
...
const EmailAnalyzer: React.FC<EmailAnalyzerProps> = ({ onStatusChange }) => {
  ...
  const [status, setInternalStatus] = useState<'IDLE' | 'ANALYZING' | 'RESULT' | 'ERROR'>('IDLE');
  
  const setStatus = (newStatus: 'IDLE' | 'ANALYZING' | 'RESULT' | 'ERROR') => {
  ...
  const handleAnalyze = async () => {
    if (!subject.trim() && !body.trim()) {
      setErrorMessage('Please enter an email subject or body to analyze.');
      setStatus('ERROR');
      return;
    }
    ...
    try {
      ...
      setTimeout(() => {
        setErrorMessage(error.message || 'Unable to connect to the MailShield inference service.');
        setStatus('ERROR');
      }, 1000);
    }
  };
  ...
  return (
    <div className="w-[92%] max-w-[740px] mx-auto py-12 relative z-20">
      ...
      >
        ...
        <div className="mb-8">
          <h2 className="text-2xl font-bold tracking-wide mb-1 drop-shadow-sm">Email Spam Analyzer</h2>
          <p className="text-secondary text-sm">Inspect an email for spam and phishing signals.</p>
        </div>
          ...
          {status === 'IDLE' && (
            ...
            >
              <div className="flex flex-col gap-2 group">
                <label className="text-xs font-bold tracking-widest text-secondary uppercase group-focus-within:text-accent-green transition-colors">Email Subject</label>
                <input 
                  type="text"
                  value={subject}
              ...
              <div className="flex flex-col gap-2 group">
                <label className="text-xs font-bold tracking-widest text-secondary uppercase group-focus-within:text-accent-green transition-colors">Email Body</label>
                <textarea 
                  value={body}
              ...
              <div className="mt-8 flex flex-col items-center justify-center text-center">
                <ThunderShield />
                <p className="text-sm text-secondary dark:text-gray-400 font-medium max-w-sm mt-2">Awaiting Data. Enter a subject and body to generate a comprehensive ML threat report.</p>
        ...
        <AnimatePresence mode="wait">
          ...
          {status === 'ERROR' && (
            ...
            >
              <div className="text-accent-red font-bold tracking-widest text-sm mb-2">ANALYSIS SERVICE UNAVAILABLE</div>
              <p className="text-secondary text-sm mb-4">{errorMessage}</p>
              <button 
          ...
          {status === 'RESULT' && resultData && (
            ...
            >
              ...
              <div className="mt-12 flex justify-center">
                <motion.button 
                  onClick={handleReset}
                  whileHover={{ y: -3, boxShadow: "0 0 25px rgba(0,255,102,0.6), inset 0 0 10px rgba(0,255,102,0.4)" }}
                  whileTap={{ y: 2, boxShadow: "0 0 10px rgba(0,255,102,0.8), inset 0 0 5px rgba(0,255,102,0.5)" }}
